...
function getDefaultContentForResource(theType) {
    ...
    if (tmpType == 'Control') {
        ...
        tmpHTML.push('    }')
        tmpHTML.push('')
        tmpHTML.push('    ControlCode._onInit = _onInit;')
        tmpHTML.push('    function _onInit(){')
        tmpHTML.push('        //console.log("Ran _onInit")')
        tmpHTML.push('    }')
        tmpHTML.push('')
...
function buildApp(theAppName, scope, theOptions) {
    ...
    return new Promise( async function (resolve, reject) {
        try {
            ...
            //--- Always use 
            if (tmpOptions.deploy === true) {
                if( tmpOptions.external || (tmpAppDetails.externaldeploy && tmpAppDetails.externaldeploy[0] == 'y' )){
                    tmpLibLoc = {prefix:'.'};
                }
            }
            ...
            var tmpTitle = tmpAppDetails.title || 'Mongino';
