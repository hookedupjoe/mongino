...
var ActionAppCore = {
            ...
            getResourceCatalogURL: function(theCatName, theResType, theResName){
                ...
                if( theCatName == '__app'){
                    if( ActionAppCore.inDesigner ){
                        ...
                        tmpCatDir = ActionAppCore.util.getAppBaseURL() + 'catalog/'
                    }
                } else if( this.catIndex[theCatName]){
                    if( ActionAppCore.util.isElectron() ){
                        tmpCatDir = ActionAppCore.util.getAppBaseURL() + this.catIndex[theCatName] ;
                    } else {
                        tmpCatDir = this.catIndex[theCatName] ;
                    }
                }
    ...
    */    
    ...
    util: {
        ...
        getAppBaseURL(){
            var tmpBase = window.location.pathname;
            if( this.isElectron()){
                tmpBase = tmpBase.replace('index.html','');
            } else {
                if( tmpBase == '' ){
                    tmpBase = '.';
                } if( tmpBase == '/' ){
                    tmpBase = location.href;
                }
            }
            return tmpBase;
        },
...
function importTo(theBaseURL, theModNames, theTarget){
    var dfd = jQuery.Deferred();

    var tmpGetImport = function(theBaseURL, theModName, theTarget){
        var tmpURL = theBaseURL + theModName + '.js';
        try {
            var tmpTarget = theTarget || window || {};
    ...
    if( !(theBaseURL && theModNames) ){
        dfd.reject("theBaseURL and theModName required");
    } else {
        var tmpModNames = theModNames;
        if( typeof(tmpModNames) == 'string'){
            tmpModNames = [tmpModNames];
        }
        var tmpToLoad = [];
        for( var iPos in tmpModNames ){
            var tmpModName = tmpModNames[iPos];
            tmpToLoad.push(tmpGetImport(theBaseURL,tmpModName, theTarget))
        }
  ...
  //--- ToDo: Move this to use generic method to import all and how to not have to adjust the url
   ...
   */
    ...
    */
    ...
    me.getActionFromObj = function (theObj, theOptionalTag) {
        ...
        //--- Update to not read the action of a form as an Mongino action
        //JF - 10-28-20
